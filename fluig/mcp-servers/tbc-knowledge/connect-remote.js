 ...
 *
 * Lê a API key (opcional) de:
 *   1. Variável de ambiente TBC_API_KEY
 *   2. Campo api_key ou tbc_api_key em ~/.config/tbc/dev-config.json
 * Se presente, envia Authorization: Bearer <key>. Se ausente, conecta sem auth (compat. interna).
 */
...
function getApiKey() {
  ...
  if (existsSync(CONFIG_PATH)) {
    try {
      ...
      if (config.tbc_api_key) return config.tbc_api_key;
    } catch { /* ignore */ }
  }
  return null;
}
...
async function main() {
  ...
  if (apiKey) {
    ...
    log('API key configurada — enviando Authorization header');
  }
