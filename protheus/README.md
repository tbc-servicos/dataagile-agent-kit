...
Conecta automaticamente ao **MCP Server remoto** com a Knowledge Base de fontes padrão TOTVS.
...
- Node.js 18+
- Email corporativo cadastrado no painel admin TBC
...
O MCP autentica pelo email cadastrado no painel admin TBC. **Sem este passo, o MCP não conecta.**
...
> O email precisa estar cadastrado no painel admin TBC. Solicite ao administrador em `https://mcp.totvstbc.com.br/admin`.
...
```json
{
  "mcpServers": {
    "tbc-knowledge": {
      "command": "bash",
      "args": ["/caminho/para/claude_skills/protheus/mcp-servers/advpl-knowledge/start.sh"],
      "env": {
...
| `/protheus:specialist` | Especialista ADVPL: consulta fontes padrão + TDN |
...
| Comando | Descrição |
|---------|-----------|
| `/protheus:update-knowledge` | Atualiza a Knowledge Base de fontes padrão |
...
O servidor MCP expõe 9 ferramentas de busca nos fontes padrão TOTVS:
...
│   └── update-knowledge/          # admin: atualiza KB
