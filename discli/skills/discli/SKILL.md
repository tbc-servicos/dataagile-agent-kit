...
## Resolução de usuários por nome

Quando o usuário mencionar um membro pelo username (ex: "fulano"), **sempre resolver o ID antes de mencionar**:

```bash
# 1. Buscar o membro pelo username
discli member list --format yaml | grep -i "fulano" -A 3

# 2. Usar o ID retornado para construir a menção: <@ID>
discli msg send <canal> "Ei <@USER_ID> ..."
```

Nunca enviar `@nome` como texto literal — o Discord só processa menções no formato `<@ID>`.

## Notes

- Channel/role/member names are resolved case-insensitively.
- You can use IDs instead of names to avoid ambiguity — **prefer IDs when multiple channels share the same name** (e.g. several `#geral` channels in different categories).
...
- **Deleting a category requires it to be empty first** — move or delete all child channels before running `channel delete` on a category, or Discord will reject the request.
- **Messages are preserved on rename and move** — only a `channel delete` permanently removes message history. Safe to rename/move freely.
- **`msg read` uses `-n`, not `--limit`** — `discli msg read <channel> -n 15` is correct; `--limit` does not exist.
- **`msg list` does not exist** — the correct subcommand is `msg read`.
